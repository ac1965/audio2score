...
- Python 3.10 系（BasicPitch・Demucs の安定動作のため必須）
  - pyenv による Python 3.10.x のインストールを推奨
  - Homebrew python3 は 3.12 系になるため非推奨
...
本ツールは以下の Python ライブラリを利用します。すべて ``pip install .`` によって自動インストールされます。
...
| torchcodec>=0.8.1 | torchaudio の save() 呼び出しで必要になる場合あり |

** Demucs Model

初回実行時に以下のモデルがダウンロードされます：
...
以下のコマンドで CLI が動作することを確認してください。

#+begin_src bash
  mscore --help      # MuseScore 3
  # または
  musescore4 --help  # MuseScore 4
#+end_src
...
- Intel MKL（numpy MKL 版）
- Python 3.10 + numba による librosa 高速化
- Apple Silicon 上での torch-neon 最適化
...
| C     | Normalize WAV  | [[file:./build/おはよう.normalized.wav]]                       |
| M1    | Main MIDI Only | [[file:./build/midi/main/おはよう.normalized_basic_pitch.mid]] |
| Merge | OUT_STEMS       | [[file:./build/おはよう.normalized_basic_pitch.musicxml]]      |
|       | PDF_MAIN        | [[file:./build/おはよう.normalized_basic_pitch.pdf]]           |
